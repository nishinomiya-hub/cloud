...
// 处理登录请求并返回响应PDU
PDU* MyTcpSocket::handleLoginRequest(PDU* pdu,QString& m_strName)
{
    char caName[32] = {'\0'};
    char caPwd[32] = {'\0'};
    strncpy(caName, pdu->caData, 32);
    ...
    if (ret)
    {
        ...
        // 确保用户文件夹存在
        QDir userDir(strUserRootPath);
        if (!userDir.exists())
        {
            bool dirCreated = userDir.mkpath("."); // 创建目录
            qDebug() << "创建用户文件夹：" << strUserRootPath << " 结果：" << dirCreated;
            if (!dirCreated)
            {
...
{
    QStringList strList = DBOperate::getInstance().handleFlushFriendRequest(pdu->caData); // 查询请求，查询数据库所有在线用户
...
{
    char *caCurDir=new char[pdu -> uiMsgLen];
...
{
    PDU* resPdu = mkPDU(0);
    char *strDelPath=new char[pdu -> uiMsgLen];
...
{
    char *strEntryPath=new char[pdu -> uiMsgLen]; // 进入文件夹路径
...
{
    char *strPrePath=new char[pdu -> uiMsgLen]; // 进入文件夹路径
...
PDU* handleMoveFileRequest(PDU* pdu) {
    ...
    char *caOldDir=new char[iOldDirSize + 33]; // +33是为了拼接文件名
    char *caDesDir=new char[iDesDirSize + 33];
...
// 分享文件请求处理
PDU* handleShareFileRequest(PDU* pdu, const QString& strSouName) {
    ...
    char *caFilePath=new char[iFilePathLen];
...
PDU* handleShareFileNoteRespond(PDU *pdu)
{
    ...
    char *caOldPath=new char[iOldPathLen];
    char *caNewDir=new char[iNewPathLen];
