...
void DBOperate::init()
{
    ...
    m_db.setDatabaseName("mysql"); // 数据库名
...
bool DBOperate::handleRegist(const char *name, const char *pwd)
{
    ...
    // 数据插入数据库
    QString strQuery = QString("insert into userInfo(name, psw) values('%1', '%2')").arg(name).arg(pwd);
    QSqlQuery query;
...
bool DBOperate::handleLogin(const char *name, const char *pwd)
{
    ...
    // 数据库查询
    QString strQuery = QString("select * from userInfo where name = \'%1\' and psw = \'%2\' "
    ...
    if(query.next()) // 每次调用next都会读取一条数据，并将结果放入query中，返回值为true，无数据则返回false
    {
        ...
        strQuery = QString("update userInfo set online = 1 where name = \'%1\' and psw = \'%2\' ").arg(name).arg(pwd);
